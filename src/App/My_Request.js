...
function Request() {
    ...
    const handleDelete = (request_id) => {
        Axios.post("http://localhost:8000/login/Main_Dashboard/deleteReq", { request_id })
          .then((response) => {
            if (response.data.status) {
              const updatedCards = req.filter((req) => req.request_id !== request_id);
              setReq(updatedCards);
            } else {
              console.log("Request not deleted:", response.data.message);
            }
          })
          .catch((error) => {
